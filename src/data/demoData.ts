...
const BASE_MONTH: Entry[] = [
  ...
  { day: 5, kind: 'expense', description: 'Aluguel', amount: 1850, categoryId: 'aluguel', account: 'corrente' },
  { day: 8, kind: 'expense', description: 'Supermercado', amount: 642.9, categoryId: 'supermercado', account: 'cartao' },
  { day: 9, kind: 'expense', description: 'Internet e telefone', amount: 149.9, categoryId: 'internet', account: 'cartao' },
  { day: 10, kind: 'expense', description: 'Conta de luz', amount: 187.4, categoryId: 'eletricidade', account: 'corrente' },
  { day: 12, kind: 'expense', description: 'Combustível', amount: 260, categoryId: 'combustivel', account: 'cartao' },
  { day: 14, kind: 'expense', description: 'Almoço no trabalho', amount: 218.5, categoryId: 'restaurantes', account: 'cartao' },
  { day: 15, kind: 'contribution', description: '', amount: 700, categoryId: 'meta', goalIndex: 0 },
  { day: 16, kind: 'expense', description: 'Academia', amount: 129.9, categoryId: 'academias', account: 'cartao' },
  { day: 20, kind: 'expense', description: 'Farmácia', amount: 96.3, categoryId: 'farmacia', account: 'cartao' },
  { day: 22, kind: 'expense', description: 'Cinema e jantar', amount: 178, categoryId: 'cinema-e-shows', account: 'cartao' },
  { day: 24, kind: 'expense', description: 'Supermercado', amount: 398.2, categoryId: 'supermercado', account: 'cartao' },
  { day: 25, kind: 'contribution', description: '', amount: 400, categoryId: 'meta', goalIndex: 1 },
  { day: 27, kind: 'expense', description: 'Transporte por app', amount: 143.6, categoryId: 'taxi', account: 'cartao' },
]
...
const SUBSCRIPTIONS: { day: number; description: string; amount: number; categoryId: string }[] = [
  { day: 20, description: 'Streaming de vídeo', amount: 55.8, categoryId: 'streaming-de-video' },
  { day: 8, description: 'Plano de celular', amount: 79.9, categoryId: 'celular' },
  { day: 15, description: 'Streaming de música', amount: 21.9, categoryId: 'streaming-de-musica' },
...
const INSTALLMENTS: {
  ...
  { day: 12, description: 'Notebook', parcela: 416.5, parcelas: 10, pagas: 6, categoryId: 'eletronicos' },
  { day: 6, description: 'Sofá da sala', parcela: 289.9, parcelas: 6, pagas: 2, categoryId: 'utilidades-domesticas' },
  { day: 22, description: 'Curso de inglês', parcela: 197, parcelas: 4, pagas: 4, categoryId: 'cursos-online' },
]
...
const MONTHLY_VARIATION: { factor: number; extra: Entry[] }[] = [
  {
    ...
    factor: 1,
    extra: [
      { day: 12, kind: 'expense', description: 'IPVA e seguro do carro', amount: 1900, categoryId: 'impostos-do-veiculo', account: 'corrente' },
  ...
  },
  {
    factor: 0.97,
    extra: [
      { day: 13, kind: 'expense', description: 'Consulta médica', amount: 320, categoryId: 'hospitais', account: 'cartao' },
    ],
  },
  {
    factor: 1.05,
    extra: [
      ...
      { day: 21, kind: 'expense', description: 'Manutenção do carro', amount: 680, categoryId: 'manutencao-do-veiculo', account: 'cartao' },
    ],
  },
  {
    factor: 0.92,
  ...
  },
  {
    ...
    extra: [
      ...
      { day: 23, kind: 'expense', description: 'Passagem aérea', amount: 890, categoryId: 'aereas', account: 'cartao' },
    ],
  },
  {
    factor: 0.86,
    extra: [
      { day: 7, kind: 'income', description: 'Rendimento da reserva', amount: 210, categoryId: 'dividendos', account: 'corrente' },
...
export function createDemoData(): FinanceData {
  ...
  return {
    ...
    goals,
    transactions,
    // Sem limites: a tela de Orçamento saiu do produto, e um teto que ninguém
    // vê nem edita não tem o que demonstrar.
    budgets: {},
  }
}
